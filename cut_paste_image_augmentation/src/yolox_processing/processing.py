...
def postprocess(
    ...
    if create_visualization == False:
        if dets is None:
            return None  # no detections to draw
        final_boxes, final_scores, final_cls_inds = decompose_detections(dets)
        bboxes, names, conf_values = get_bboxes(
            image,
            final_boxes,
            final_scores,
            final_cls_inds,
        ...
        return bboxes, names, conf_values

    elif create_visualization == True:
        if dets is None:
...
def get_bboxes(img, boxes, scores, cls_ids, conf=0.5, class_names=None):
    bboxes = []
    names = []
    conf_values = []
    for i in range(len(boxes)):
        ...
        names.append(name)
        conf_value = round(score, 3)
        conf_values.append(str(conf_value))
    return bboxes, names, conf_values
